...
export const ToolCallBox = React.memo<ToolCallBoxProps>(
  ({
    ...
    const { name, args, result, status } = useMemo(() => {
      // Handle case where args might be a string or have numeric keys (from stringified JSON)
      ...
      // If args has only numeric keys (0, 1, 2...), it's likely a stringified JSON
      // that was iterated character-by-character. Try to reconstruct and parse it.
      ...
      if (keys.length > 0 && keys.every(k => /^\d+$/.test(k))) {
        try {
          const reconstructedString = keys
            ...
            .map(k => parsedArgs[k])
            .join('');
          parsedArgs = JSON.parse(reconstructedString);
        } catch {
          // If reconstruction fails, show as a single value
          parsedArgs = { raw: keys.map(k => parsedArgs[k]).join('') };
        }
      }
